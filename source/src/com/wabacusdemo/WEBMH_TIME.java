 ...
 *
 */
public class WEBMH_TIME extends AbsInterceptorDefaultAdapter
{
	/**
	 * 每行数据显示到页面之前执行的函数
	 * @创建者：zhouhl
	...
	public Object beforeLoadData(ReportRequest rrequest, ReportBean rbean,
		...
		if (sql.contains("1=1") && !sql.contains("t.time >= to_date") && !sql.contains("t.time <= to_date")) {
			...
			sql = sql.replaceAll("1=1", "t.time >= to_date('" + time1 + "','yyyy-MM-dd hh24:mi:ss') and t.time <= to_date('" + time2 + "','yyyy-MM-dd hh24:mi:ss')");
		}
