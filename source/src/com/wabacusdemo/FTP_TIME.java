 ...
 *
 */
public class FTP_TIME extends AbsInterceptorDefaultAdapter
{
	/**
	 * 每行数据显示到页面之前执行的函数
	 * @创建者：zhouhl
	...
	public Object beforeLoadData(ReportRequest rrequest, ReportBean rbean,
		...
		if (sql.contains("1=1") && !sql.contains("ftplink.linktime >= to_date") && !sql.contains("ftplink.linktime <= to_date")) {
			...
			String time = sf1.format(cal.getTime());
			
			//设定开始时间
			String time1 = sf2.format(cal.getTime());
			Calendar ca2 = Calendar.getInstance();
			
			//设定结束时间
			SimpleDateFormat sf3 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String time2 = sf3.format(ca2.getTime());
			rrequest.setAttribute("txtbegin", time1);
			rrequest.setAttribute("txtend", time2);
			
			//String now = sf.format(date) + ":00:00";
			sql = sql.replaceAll("1=1", "((ftplink.linktime >= to_date('" + time1 + "','yyyy-MM-dd hh24:mi:ss')) and (ftplink.linktime <= to_date('" + time2 + "','yyyy-MM-dd hh24:mi:ss')))");
		}
