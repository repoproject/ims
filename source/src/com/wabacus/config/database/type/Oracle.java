...
public class Oracle extends AbsDatabaseType
{
    ...
    public void constructInsertSql(String configInsertSql,ReportBean rbean,String reportTypeKey,InsertSqlActionBean insertSqlBean)
    {
        ...
        boolean isAutoReportData=insertSqlBean.getOwnerGroupBean().getOwnerUpdateBean().isAutoReportdata();
        ...
        {//没有指定要更新的字段，则将所有符合要求的从数据库取数据的<col/>全部更新到表中
            ...
            {
                EditableReportParamBean paramBean=insertSqlBean.createParamBeanByColbean(cbean.getProperty(),reportTypeKey,false,false);
        ...
        }else
        {
            ...
            for(String configColPropertyTmp:lstInsertCols)
            {
                ...
                if(idxequals>0)
                {
                    ...
                    Object paramObjTmp=insertSqlBean.createEditParams(columnvalue,reportTypeKey);
                ...
                }else
                {
                    ...
                    if(isAutoReportData)
                    {
                        ...
                        lstParams.add(processLobTypeInsert(mLobParamsBean,cb.getColumn(),insertSqlBean.createParamBeanByColbean(configColPropertyTmp,
        ...
        if(sqlBuffer.charAt(sqlBuffer.length()-1)==',') sqlBuffer.deleteCharAt(sqlBuffer.length()-1);
        sqlBuffer.append(")");
        insertSqlBean.addInsertSqlActionBean(sqlBuffer.toString(),lstCommonTypeParamsBean,insertSqlBean.getReturnValueParamname());
        if(mLobParamsBean.size()>0)
        {
            ...
            whereclause=insertSqlBean.getOwnerGroupBean().getOwnerUpdateBean().parseStandardEditSql(whereclause,lstDynParamsInWhereClause,reportTypeKey);
            lstParamsBean2.addAll(lstDynParamsInWhereClause);
            sqlBuffer.append(" ").append(whereclause).append(" for update");
            insertSqlBean.addInsertSqlActionBean(sqlBuffer.toString(),lstParamsBean2,null);
        }
    }
    ...
    public List<UpdateSqlActionBean> constructUpdateSql(String configUpdateSql,ReportBean rbean,String reportTypeKey,UpdateSqlActionBean updateSqlBean)
    {
        ...
        {//没有指定要更新的字段，则将所有从数据库取数据的<col/>（不包括displaytype为hidden的<col/>）全部更新到表中
            ...
            for(ColBean cbean:rbean.getDbean().getLstCols())
            {
                EditableReportParamBean paramBean=updateSqlBean.createParamBeanByColbean(cbean.getProperty(),reportTypeKey,false,false);
                if(paramBean==null) continue;
        ...
        }else
        {
            ...
            for(String updatecol:lstUpdateCols)
            {
                ...
                if(idxequals>0)
                {
                    ...
                    Object paramObjTmp=updateSqlBean.createEditParams(columnvalue,reportTypeKey);
                    if(paramObjTmp==null) continue;
                    IDataType datatypeObj=null;
                    if(Tools.isDefineKey("#",columnvalue))
                    {//是从<params/>中定义的变量中取值
                ...
                }else
                {
                    ...
                    if(isAutoReportData)
                    {
                        ...
                        EditableReportParamBean paramBean=updateSqlBean.createParamBeanByColbean(cb.getProperty(),reportTypeKey,true,true);
        ...
        if(mLobParamsBean!=null&&mLobParamsBean.size()>0)
        {
            ...
            List<EditableReportParamBean> lstParamsBean2=new ArrayList<EditableReportParamBean>();
            StringBuffer sqlSelectLob=new StringBuffer("select ");
            StringBuffer sqlUpdateLob=new StringBuffer("update ");
