...
public class SaveButton extends AbsEditableReportButton
{
    public SaveButton(IComponentConfigBean ccbean)
    {
        super(ccbean);
    }
    ...
    protected String getClickEvent(ReportRequest rrequest,String params)
    {
        ...
        {
            lstBindedReportBeans=ersqlbean.getLstSaveBindingReportBeans();
        }
        if(lstBindedReportBeans!=null&&lstBindedReportBeans.size()>0)
        {
            for(ReportBean rbBindedTmp:lstBindedReportBeans)
            {
                paramsBuf.append(",{reportid:'").append(rbBindedTmp.getId()).append("',updatetype:'save'}");
            }
        }
        paramsBuf.append("]}");
